...
// We build a chainable fluent mock. Each test can push results onto the queue
// so calls are dequeued in order.
...
function makeChain(queue: MockResult[]) {
  ...
  // Support .select('id', { count:'exact', head:true }) pattern — same chain
  return chain
}

// Server client only needs auth.getUser + profiles.select
...
// generateId returns predictable value in tests
...
// ══════════════════════════════════════════════════════════════════════════════
// Route: GET/POST /api/content-plans
// ══════════════════════════════════════════════════════════════════════════════
...
describe('POST /api/content-plans', () => {
  ...
  it('404 when client not found', async () => {
    const { POST } = await import('./route')
    queueAdminAuth()
    adminQueue.push({ data: null, error: null }) // client lookup
    const res = await POST(makeReq('POST', 'http://x/api/content-plans', { client_id: 'c1', month: '2025-07' }))
...
// ══════════════════════════════════════════════════════════════════════════════
// Route: PATCH/DELETE /api/content-plans/[id]
// ══════════════════════════════════════════════════════════════════════════════
...
describe('DELETE /api/content-plans/[id]', () => {
  ...
  it('409 when plan is active (not draft)', async () => {
    const { DELETE } = await import('./[id]/route')
    queueAdminAuth()
    adminQueue.push({ data: { status: 'active' }, error: null }) // fetch plan
    const res = await DELETE(makeReq('DELETE', 'http://x/api/content-plans/p1'), await makeIdParams('p1'))
    expect(res.status).toBe(409)
  ...
  it('200 deletes a draft plan', async () => {
    const { DELETE } = await import('./[id]/route')
    queueAdminAuth()
    adminQueue.push({ data: { status: 'draft' }, error: null }) // fetch
    adminQueue.push({ data: null, error: null })                // delete
    const res = await DELETE(makeReq('DELETE', 'http://x/api/content-plans/p1'), await makeIdParams('p1'))
    expect(res.status).toBe(200)
...
// ══════════════════════════════════════════════════════════════════════════════
// Route: GET/POST /api/content-plans/[id]/items
// ══════════════════════════════════════════════════════════════════════════════
...
describe('POST /api/content-plans/[id]/items', () => {
  ...
  it('404 when plan not found', async () => {
    const { POST } = await import('./[id]/items/route')
    queueAdminAuth()
    adminQueue.push({ data: null, error: { message: 'not found' } }) // plan fetch
    const res = await POST(makeReq('POST', 'http://x', { content_type: 'reel', publish_date: '2025-07-15' }), await makeItemsParams('p1'))
    expect(res.status).toBe(404)
  })
  ...
  function queueSuccessfulItemCreation(opts: {
    ...
    adminQueue.push({ data: null, error: null, count }) // sequence count
    adminQueue.push({ data: { id: 'item-1', content_type: contentType }, error: null }) // item insert
    ...
    adminQueue.push({ data: { id: 'item-1', content_type: contentType, task: { id: 'task-id-123' } }, error: null }) // full item fetch
  }
  ...
  it('does not activate plan when already active', async () => {
    ...
    const res = await POST(makeReq('POST', 'http://x', { content_type: 'reel', publish_date: '2025-07-15T00:00:00.000Z' }), await makeItemsParams('p1'))
    expect(res.status).toBe(201)
    // If plan activation was queued for 'active' plan, queue would have extra item → next test would fail (good signal)
  })

  it('title format: ClientName_Reel_01_July2025', async () => {
    const { POST } = await import('./[id]/items/route')
    queueAdminAuth()
    // We capture the insert call by overriding createAdminClient temporarily
    ...
    // Title is checked inside the item insert payload
  ...
  it('500 and rolls back item when task creation fails', async () => {
    ...
    adminQueue.push({ data: null, error: null, count: 0 }) // sequence
    adminQueue.push({ data: { id: 'item-1' }, error: null }) // item insert
...
// ══════════════════════════════════════════════════════════════════════════════
// Route: PATCH/DELETE /api/content-plans/[id]/items/[itemId]
// ══════════════════════════════════════════════════════════════════════════════
...
describe('PATCH /api/content-plans/[id]/items/[itemId]', () => {
  ...
  it('recalculates due date when publish_date changes (reel SLA=3)', async () => {
    const { PATCH } = await import('./[id]/items/[itemId]/route')
    queueAdminAuth()
    // fetch item to get sla_days + task_id
  ...
  it('recalculates due date when publish_date changes (no task_id)', async () => {
    const { PATCH } = await import('./[id]/items/[itemId]/route')
    queueAdminAuth()
    adminQueue.push({ data: { sla_days: 2, task_id: null }, error: null }) // no linked task
    adminQueue.push({ data: { id: 'item-1', task: null }, error: null })   // item update
    const res = await PATCH(
      makeReq('PATCH', 'http://x', { publish_date: '2025-07-20T00:00:00.000Z' }),
      await makeItemIdParams('p1', 'item-1')
    )
...
describe('DELETE /api/content-plans/[id]/items/[itemId]', () => {
  ...
  it('404 when item not found', async () => {
    const { DELETE } = await import('./[id]/items/[itemId]/route')
    queueAdminAuth()
    adminQueue.push({ data: null, error: null }) // item fetch returns null
    const res = await DELETE(makeReq('DELETE', 'http://x'), await makeItemIdParams('p1', 'item-1'))
    expect(res.status).toBe(404)
  })
  ...
  it('200 deletes item with no linked task (task_id is null)', async () => {
    const { DELETE } = await import('./[id]/items/[itemId]/route')
    queueAdminAuth()
    adminQueue.push({ data: { task_id: null, status: 'done' }, error: null })
    adminQueue.push({ data: null, error: null }) // delete item
    const res = await DELETE(makeReq('DELETE', 'http://x'), await makeItemIdParams('p1', 'item-1'))
    expect(res.status).toBe(200)
  })

  it('can delete item at any status (pending_production, done, client_approved)', async () => {
    const { DELETE } = await import('./[id]/items/[itemId]/route')
    for (const status of ['pending_production', 'done', 'client_approved', 'revision_requested']) {
      queueAdminAuth()
