...
import { Bell, AlertCircle, Clock, Zap, X, CheckCheck, Menu, Search } from 'lucide-react'
...
export default function Header({ onMenuClick }: Props) {
  ...
  function dismissAll() { setDismissed(new Set(notifications.map((n) => n.id))); setOpen(false) }
      ...
      <header className="h-16 flex items-center justify-between px-4 md:px-6 border-b border-slate-800 bg-slate-950/50 backdrop-blur shrink-0">
        ...
        <div className="flex items-center gap-2 md:gap-3" ref={panelRef}>
          ...
          <button
            ...
            className="hidden sm:flex items-center gap-2 px-3 py-1.5 rounded-lg bg-slate-800/70 border border-slate-700/50 text-slate-400 hover:border-slate-600 hover:text-slate-200 transition-all text-xs"
          >
            <Search className="h-3.5 w-3.5" />
            <span>Search</span>
            <kbd className="text-[10px] border border-slate-700 rounded px-1 py-0.5 bg-slate-900">⌘K</kbd>
          </button>
          <button
            onClick={() => setSearchOpen(true)}
            className="sm:hidden p-2 rounded-lg text-slate-400 hover:bg-slate-800 hover:text-slate-100 transition-colors"
          >
              ...
              <div className="absolute right-0 top-12 w-80 bg-slate-900 border border-slate-700 rounded-xl shadow-2xl z-50 overflow-hidden">
                <div className="flex items-center justify-between px-4 py-3 border-b border-slate-800">
                  <span className="text-sm font-semibold text-slate-100">
                    Notifications
                    {unread > 0 && (
                      <span className="ml-2 text-xs px-1.5 py-0.5 rounded-full bg-red-500/20 text-red-400 font-medium">{unread}</span>
                    )}
                  </span>
                  {unread > 0 && (
                    <button onClick={dismissAll} className="flex items-center gap-1 text-xs text-slate-400 hover:text-slate-200 transition-colors">
                ...
                <div className="max-h-80 overflow-y-auto divide-y divide-slate-800">
                  {visible.length === 0 ? (
                    ...
                    visible.map((n) => (
                      <div key={n.id} className="flex items-start gap-3 px-4 py-3 hover:bg-slate-800/60 transition-colors group">
                        {severityIcon[n.severity]}
                        <div className="flex-1 min-w-0 cursor-pointer" onClick={() => { dismiss(n.id); setOpen(false); if (n.link) router.push(n.link) }}>
                          <p className="text-xs font-semibold text-slate-200">{n.title}</p>
                          <p className="text-xs text-slate-400 truncate mt-0.5">{n.message}</p>
                        </div>
                        <button onClick={() => dismiss(n.id)} className="opacity-0 group-hover:opacity-100 transition-opacity p-0.5 rounded text-slate-600 hover:text-slate-300">
                          <X className="h-3 w-3" />
                        </button>
                      </div>
                    ))
